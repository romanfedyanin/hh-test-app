...
- (id)initWithView:(UIView *)view andStyle:(MBProgressHUDStyle)hudStyle {
	NSAssert(view, @"View must not be nil.");
    self.style = hudStyle;
	id me = [self initWithFrame:view.bounds];
	// We need to take care of rotation ourselfs if we're adding the HUD to a window
...
- (void)layoutSubviews {
    ...
    if (style == MBProgressHUDLeft) {
        ...
        labelSize.width = MIN(labelSize.width + indicatorF.size.width, maxWidth);
        totalSize.width = MAX(totalSize.width, labelSize.width);
        totalSize.height = MAX(labelSize.height, indicatorF.size.height);
        ...
        [paragraphStyle setLineBreakMode:detailsLabel.lineBreakMode];
        
        CGSize detailsLabelSize = CGRectIntegral([detailsLabel.text boundingRectWithSize:maxSize options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) attributes:@{NSFontAttributeName:detailsLabel.font,NSParagraphStyleAttributeName:paragraphStyle} context:nil]).size;
        ...
        if (labelSize.height > 0) {
            ...
            labelDelta = labelSize.width;
        }
        else {
            totalSize.width += 2 * kPadding;
            labelDelta = 0.0f;
        }
        totalSize.height += 2 * kPadding;
        
        // Position elements
        CGFloat yPos = roundf((bounds.size.height - indicatorF.size.height) / 2) + yOffset;
        CGFloat xPos = xOffset;
        indicatorF.origin.y = yPos;
        indicatorF.origin.x = roundf((bounds.size.width - indicatorF.size.width - labelDelta) / 2) + xPos;
        indicator.frame = indicatorF;
    
        CGRect labelF;
        
        if (labelSize.height > indicatorF.size.height) {
            yPos = yPos - roundf((labelSize.height - indicatorF.size.height) / 2);
        }
        
        labelF.origin.y = yPos;
        labelF.origin.x = indicatorF.origin.x + indicatorF.size.width + kPadding + xPos;
